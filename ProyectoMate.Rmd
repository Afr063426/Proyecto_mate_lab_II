...
salario_referencia <- function(antiguedad, salario, k){
    ...
    salario_ref <- mean((1.025)^t * (1.04)^ (t+s)) * salario
...
if(sexo == "M"){
  ...
  costos_primer_criterio <- salario_ref * anualidades_primer_criterio
  ...
  denominadores <- apply(X = probs_condicionales, MARGIN = 1, FUN=sum) %>% unlist()
  # Tercero, se determinan las probabilidades condicionales dividiendo cada fila (numerador) entre el denominador respectivo
  
  probs_condicionales <- sweep(x=numeradores, MARGIN = 1, STATS = denominador, FUN = '/' )
...
```{r}
costo_afiliado_estocastico(x = 55, sexo = "F", salario = 1000000, antiguedad = 10)
```

```{r}
t <- proc.time()

costo_estoc <- 1:nrow(Afiliados_31_12_2021)
Nsim <- 5000

for (i in  1:nrow(Afiliados_31_12_2021)) {
print(i)
x <- Afiliados_31_12_2021$Edad[i]
...
for (k in 1:Nsim) {
  sum <- sum + costo_afiliado_estocastico(x, sexo, salario, antiguedad)
}

costo_estoc[i] <- sum/Nsim
