...
beneficio <- function(antiguedad, decremento, salario, k) {
  ...
  return(salario_referencia(antiguedad, salario, k) * porcentaje_de_salario / 12)
}
...
```{r}
DecrementosIV <- function(Edad, hombre, antiguedad, ultimo_salario, tasa_descuento, tasa_cre_salario, decremento) {
  v <- (1) / (1 + tasa_descuento)
...
DecrementosIV2 <- function(Edad, hombre, antiguedad, ultimo_salario, tasa_descuento, tasa_cre_salario, decremento, n = 0) {
...
DecrementoM <- function(Edad, hombre, antiguedad, ultimo_salario, tasa_descuento, tasa_cre_salario) {
  v <- (1) / (1 + tasa_descuento)
  ...
  if (hombre) {
    ...
    for (k in 0:(77 - Edad)) {
      MontoAPagar <- beneficio(antiguedad, 4, ultimo_salario, k) * (1 + 0.04)^(vectorAnualmensual) * 2^vectoraguinaldo
      suma1 <- sum((v)^(vectorMensual) * MontoAPagar)
      PV <- PV + suma1 * v^(k + 1) * pxt(MDListaDeTablasMujeres[[Edad + 1]], x = Edad, t = k) *
        qxt(MDListaDeTablasMujeres[[Edad + 1]], x = Edad + k, t = 1, decrement = 4)
    }
  }
  return(PV)
}
...
```{r}
# Esta es la funcion de decremento por muerte
DecrementoM2 <- function(Edad, hombre, antiguedad, ultimo_salario, tasa_descuento, tasa_cre_salario, n = 0) {
  v <- (1) / (1 + tasa_descuento)
  edades <- min(Edad):max(Edad)
  montos_pension <- list()
  # La funcion beneficio aun no ha sido definida
...
```{r}
DecrementoM2(afil_hombres$Edad[1], TRUE, afil_hombres$Antiguedad[1], afil_hombres$Salario[1], 0.07, 0)
```
...
# Por invalidez
salida_invalidez_h <- DecrementosIV2(edades_h, TRUE, antiguedades_h, salarios_h, tasa_descuento, tasa_crecimiento_r, 1)

# Por jubilacion
salida_jubilacion_h <- DecrementosIV2(edades_h, TRUE, antiguedades_h, salarios_h, tasa_descuento, tasa_crecimiento_r, 2)

# Por muerte
salida_muerte_h <- DecrementoM2(edades_h, TRUE, antiguedades_h, salarios_h, tasa_descuento, tasa_crecimiento_r)
...
salida_invalidez_m <- DecrementosIV2(edades_m, FALSE, antiguedades_m, salarios_m, 0.07, 0, 1)

salida_jubilacion_m <- DecrementosIV2(edades_m, FALSE, antiguedades_m, salarios_m, 0.07, 0, 2)

salida_muerte_m <- DecrementoM2(edades_m, FALSE, antiguedades_m, salarios_m, tasa_descuento, tasa_crecimiento_r)
