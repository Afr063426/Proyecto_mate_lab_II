...
# Tabla de múltiples decrementos para usar con lifecontingencies

TablaMDHombres <- new("mdt", table=tabla_decrementos_hombres)
TablaMDMujeres <- new("mdt", table=tabla_decrementos_mujeres)
...
Para los personas afiliadas al plan ofrecido por la Compañía, se implementa una modificación de uno de los algortimos descritos en Racinello (1988) bajo un modelo de múltiples decrementos. El mencionado algoritmo permite simular la variable aleatoria $Z_{x}$ del tiempo de trabajo residual de un empleado activo de edad $(x)$. En esencia, el método proporciona un criterio para, en un primer paso y dada una muestra de una distribución $\mathcal{U}(0,1)$, decidir si una persona de edad $(x)$ se llega a jubilar, en cuyo caso resta solamente simular su tiempo de vida residual. En su defecto, el algoritmo proporciona una regla para asignar, de acuerdo a una segunda muestra de una distribución $\mathcal{U}(0,1)$, cuál de los decrementos restantes (muerte, invalidez o retiro) sucedió. Como puede comprobarse, el método requiere a lo sumo dos muestras $\mathcal{U}(0,1)$ en aras de obtener una muestra de $Z_{x}$.
...
costo_afiliado_estoc(x, probs, antiguedad, tasa_descuento, m, inflacion, v){
  
  z <- runif(1)
  
  # Criterio del algoritmo en caso de jubilación
  
  if(z > probs[85-x+1]) {
    # Se simula el tiempo de vida residual, pues se sabe que la persona se jubiló
    
  }
  
#  else{
    
    # Criterio del algoritmo en caso de no jubilación
...
pension_heredada <- function(edad, monto, activo, tasa_descuento, m, inflacion, v) {
  ...
  j <- 1 / m
  i <- 1
  # Se verifica que la persona no haya tenido el beneficio mas de 10 ambos
  if (activo < 10) {
...
```{r}
pension_pen_m <- pension_pen(pen_mujeres, FALSE, 0.07, 12, 1)
pension_pen_h <- pension_pen(pen_hombres, TRUE, 0.07, 12, 10)
```
