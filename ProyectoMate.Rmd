...
Se programa primero la función `costo_afiliado_estoc` que retorna una simulación estocástica del costo que representa un afiliado con características:

-`x`: edad actual de la persona
-`sexo`: sexo de la persona
-`salario_act`: salario de referencia
-`antiguedad_act`: antigüedad de la persona en la Compañía

```{r}

# Implementación del algoritmo descrito en Racinello (1988)

costo_afiliado_estoc <- function(x, sexo, salario_act, antiguedad_act){ 
  ...
  q <- 1-prod(1-qxs[(x+1):(xi),5]) # P(Wx <= xi-x-1)
  
  if(u > q) {# Criterio del algoritmo en caso de jubilación obligatoria
    
    salrio_ref <-
  
    antiguedad <- antiguedad_act+xi-x
    
    porcentaje <- porcentaje_de_pension(antiguedad)
    
    # Se simula la vida residual (con el modelo simple asociado)
    
    Kx <- rLife(1, xi, k = 12, type = "Kx")
    
    costo <- porcentaje * salario_ref * annuity(i = 0.07, n=Kx, m = (xi-x), k = 12, type = "due")
    
  }else{# Criterio del algoritmo en caso de no jubilación obligatoria  
  ...
  # vector probsabilidades condicionales
  
  qbar <- qxs[(x+k+1), 1:4]/denominador 
  
  # Se determina el decremento acontecido
  
  if(u<=qbar[1]){# Invalidez
  
    
    Kx <- rLife(1, q4_prima, x+k, type = "Kx")
    
    porcentaje <- porcentaje_de_pension(antiguedad)
    
    costo <- 0.6 * porcentaje * salario_ref * annuity(i = 0.07, n=Kx, m = (x+k), k = 12, type = "due")
    
    
  }else if(qbar[1]<u && u<=sum(qbar[1:2])){ # Jubilación
    
  Kx <- rLife(1, q4_prima, x+k, type = "Kx")
    
  porcentaje <- porcentaje_de_pension(antiguedad)
    
  costo <- porcentaje * salario_ref * annuity(i = 0.07, n=Kx, m = (x+k), k = 12, type = "due")
    

  }else if(sum(qbar[1:2])<u && u<=sum(qbar[1:3])){ # Retiro
  # En este caso no se otorga beneficio
  ...
  costo <- 0.2 * porcentaje * salario_ref * annuity(i = 0.07, n=10, m = (x+k), k = 12, type = "due")
...
```{r}
# costo_afiliado_estoc(x = 55, sexo = "F", salario_ref = 1000000, antiguedad = 5)
```
