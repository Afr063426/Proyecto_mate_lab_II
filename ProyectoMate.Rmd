  ...
  function(Edad, hombre, antiguedad, ultimo_salario, tasa_descuento, tasa_cre_salario, decremento){
    ...
    if(hombre){
      for(k in 0:(115-Edad)){
        ...
        MontoAPagar <-beneficio(antiguedad, decremento, ultimo_salario, k)*(1+0.03)^(vectorAnualmensual)*2^vectoraguinaldo
        ...
        PV <- PV + suma1*v^(k+1)*pxt(MDListaDeTablasHombres[[Edad+1]],x=Edad,t=k)*
          qxt(MDListaDeTablasHombres[[Edad+1]],x=Edad+k ,t=1, decrement=decremento)
        }
...
DecrementoM <-
  function(Edad, hombre, antiguedad, ultimo_salario, tasa_descuento, tasa_cre_salario) {
    ...
    if (hombre) {
      for (k in 0:(85 - Edad)) {
        MontoAPagar <- beneficio(antiguedad, 4, ultimo_salario, k) * (1 + 0.03)^(vectorAnualmensual) * 2^vectoraguinaldo
...
sum(MontoParaMujeresDeterministicoM)
...
a<-sum(MontoParaMujeresDeterministicoV, MontoParaMujeresDeterministicoI, MontoParaMujeresDeterministicoM)
    ...
    if(hombre){
      for (i in Edades) {
        for(k in 0:(115-Edades[i])){
          ...
          anualidad <-pxt(ListaDeTablasHombres[[(Edades[i]+1)]],x=Edades[i]+k,t=vectorMensual)*(v)^(vectorMensual)
          suma1 <- anualidad * (1+0.03)^(vectorAnualmensual)*2^vectoraguinaldo
          GuardarSuma[k+1]<-suma1
        }
