...
salario_referencia <- function(antiguedad, salario, k){
    ...
    # k es el tiempo hasta que ocurre un decremento
...
Por ejemplo, si una persona de edad $50$ sufre el decremento de invelidez $2$ años despúes y tenía a la edad $50$ una antiguedad de $10$ años, entonces el salario de referencia, suponienod un salario actual de $1,000,000$ para el cálculo de su pensión es
...
if(sexo == "M"){
  ...
  salario_ref <- salario_referencia(antiguedad, salario, xi-x)
    # Se simulan los tiempos residuales de vida para los pensionados de edad máxima (xi)
    
  tiempos_residuales_vida <- rLife(n=cant_pensiones_edad_max, object = q4_prima, x=xi, k = 12, type = "curtate")
    
    anualidades_primer_criterio <- lapply(X = tiempos_residuales_vida, FUN = annuity, i=0.07, m = xi-x, k=12, type="due")
  ...
  cuantiles <- lapply(X = u[u <= FWx[xi-x-1 + (x+1)]], FUN = hallar_cuantil, distribucion = FWx)
  
  # Se determinan las probabilidades condicionales 
  
  denominador_probs_condicionales <- lapply(X=qxs[x+cuantiles+1,1:4], FUN=sum)
