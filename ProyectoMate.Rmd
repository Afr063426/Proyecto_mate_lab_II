...
```{r}
cotizacionfutura <-
  function(Edad, hombre, ultimo_salario, porcentaje_aporte=1) {
    v <- (1) / (1 + 0.07 )
    if (hombre) {
      salariosfuturos<-ultimo_salario * ((1 + 0.04)*(1+0.025))^(1:(85-Edad))*12*
        pxt1(MDListaDeTablasHombres[[Edad+1]], x = Edad, t = (1:(85-Edad)))* v^(0:(85-Edad-1))
        return(salariosfuturos*porcentaje_aporte)
      } else {
      salariosfuturos<-ultimo_salario * ((1 + 0.04)*(1+0.025))^(1:(77-Edad))*12*
        pxt1(MDListaDeTablasMujeres[[Edad+1]], x = Edad, t = (1:(77-Edad)))* v^(0:(77-Edad-1))
        return(salariosfuturos*porcentaje_aporte)
      }  
    }
...
pi<-(sum(afil_mujeres$Costo_plan)+sum(afil_hombres$Costo_plan))/(sum(unlist(SalariosM))+sum(unlist(SalariosH)))
