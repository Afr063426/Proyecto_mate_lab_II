...
# set.seed(10)
# N <- 500
# costos_individual_estoc_m <- matrix(0, nrow = nrow(afil_mujeres), ncol=4)
# t <- proc.time()
# i <-1
...
# salario <- afil_mujeres$Salario[i]
# antiguedad <-afil_mujeres$Antiguedad[i]
# costos_individual_estoc_m[i,]<- costo_afiliado_estocastico(edad,
#                                              salario,
#                                              77,
#                                              antiguedad,
#                                              cant_simulaciones = N,
...
# }
# proc.time()-t
# costos_individual_estoc_m <- as.data.frame(costos_grupal_estoc_m)
# colnames(costos_grupal_estoc_m) <- c("Invalidez", "Jubilación", "Muerte", "Total")
# write.csv(costos_individual_estoc_m,"Costos estocástico mujeres individual.csv", row.names = FALSE)
...
comma(apply(X=costos_grupal_estoc_m, MARGIN=2, FUN=sum))

comma(apply(X=costos_grupal_estoc_m, MARGIN=2, FUN=sum))+
comma(apply(X=costos_grupal_estoc_h, MARGIN=2, FUN=sum))


comma( comma(apply(X=costos_grupal_estoc_m, MARGIN=2, FUN=sum))[4]+
comma(apply(X=costos_grupal_estoc_h, MARGIN=2, FUN=sum))[4]+ 71007665316.293 )

# Err rel hombres
abs(293890129722.64-292807651363.28)/292807651363.28
# Err rel mujeres
abs(420521887316.16-424632902586.31)/424632902586.31
# Err abs total

abs(714412017038.80-717440553949.59)/717440553949.59
