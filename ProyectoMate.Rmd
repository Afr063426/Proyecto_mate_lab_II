...
```{r}
datos <- afil_mujeres %>% filter(Edad == 30) %>% select(Salario, Antiguedad)
...
costo_estocastico_grupo_etario(30, salarios, 77, antiguedades, 10000, MDListaDeTablasMujeres, ListaDeTablasMujeres)
...
costo_estocastico_grupo_etario <- function(edad, salarios, edad_maxima, antiguedades, cant_simulaciones, lista_tablas_decrementos, lista_tablas_mortalidad){
  ...
  qxs <- cbind(qxt(lista_tablas_decrementos[[x+1]], x=0:115, t=1, decrement=1),
  ...
  # RETORNAR LOS COSTOS TOTALES, PERROOOOOOOOO
...
```{r}
t <- proc.time()
costos <- future_mapply(FUN = costo_afiliado_estocastico,
