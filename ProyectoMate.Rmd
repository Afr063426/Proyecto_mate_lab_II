...
### Esperanza de vida activos

```{r}
#cálculo de las Esperanzas de vida para cada empleado

Afiliados_31_12_2021_2 <- Afiliados_31_12_2021%>%
  mutate(Exn = ifelse(Sexo == "M",unlist(map2((Edad + 1), Edad, ~ exn(object = ListaDeTablasHombres[[.x]],x = .y,type = "curtate"))),unlist(map2((Edad + 1), Edad, ~ exn(object = ListaDeTablasMujeres[[.x]],x = .y,type = "curtate")))))


Genero <- c(`F` = "Mujeres",`M` = "Hombres")

pobAfi <- Afiliados_31_12_2021_2 %>% group_by(Edad, Exn,Sexo)%>%
...
plot5 <- pobAfi %>% ggplot(aes(x=Edad, y=Cantidad, fill = Exn)) +
  geom_bar(stat="identity") +
  facet_grid(~Sexo, labeller = as_labeller(Genero))+
  theme_minimal()+scale_fill_gradient(low="red", high="yellow")+
  labs(fill='Esperanza de Vida', title = "", x = "Edad", y ="Cantidad de personas")

ggsave("plot5.eps")
plot5
```

### Esperanza de vida pensionados

```{r}
#cálculo de las Esperanzas de vida para cada jubilado 
Pensionados_31_12_2021_2 <- Pensionados_31_12_2021%>%
  mutate(Exn = ifelse(SEX == "M",unlist(map2((Edad + 1), Edad, ~ exn(object = ListaDeTablasHombres[[.x]],x = .y,type = "curtate"))),unlist(map2((Edad + 1), Edad, ~ exn(object = ListaDeTablasMujeres[[.x]],x = .y,type = "curtate")))))

  
  

pobPen <- Pensionados_31_12_2021_2 %>% group_by(Edad, Exn,SEX)%>%
  summarize(Cantidad =  n())
...
plot4 <- pobPen %>% ggplot(aes(x=Edad, y=Cantidad, fill = Exn)) +
  geom_col(stat="identity") +
  facet_grid(~SEX, labeller = as_labeller(Genero))+
  theme_minimal()+scale_fill_gradient(low="red", high="yellow")+
  labs(fill='Esperanza de Vida', title = "", x = "Edad", y ="Cantidad de personas")
plot4
ggsave("plot4.eps")
...
#Empleados
tablaFrecuenciasSalarios <- read_excel("Planilla_Compania_31_12_2021.xlsx", 
    sheet = "Frecuencia Salarios", range = "H4:L18")

tablaFrecuenciasEdad <- read_excel("Planilla_Compania_31_12_2021.xlsx", 
    sheet = "Frecuencia Edad", range = "F3:I16")

tablaPromedios <- read_excel("Planilla_Compania_31_12_2021.xlsx", 
    sheet = "Promedio Salarios", range = "F9:I12")

formattable(tablaFrecuenciasEdad)
formattable(tablaFrecuenciasEdad, 
            align =c("l","c","c", "r"), 
            list(`Rango de Edad` = formatter(
...
tablaFrecuenciasSalarios[1,1] <- "100,000-500,000"

tablaFrecuenciasSalarios[13,1] <- "6,000,0001-8,000,000"

tablaFrecuenciasSalarios <- melt(tablaFrecuenciasSalarios[-nrow(tablaFrecuenciasSalarios),c(1,2,3)] ,  id.vars = 'Rango Salarial', variable.name = 'Género')
tablaFrecuenciasSalarios$`Rango Salarial`<- factor(tablaFrecuenciasSalarios$`Rango Salarial`, levels = c("500 o menos", "500-1,000", "1,000-1,500", "1,500-2,000", "2,000-2,500", "2,500-3,000", "3,000-3,500","3,500-4,000" ,"4,000-4,500", "4,500-5,000","5,000- 5,500", "5,500- 6,000", "6,000 o más"))

plot1 <- tablaFrecuenciasSalarios%>%
ggplot( aes( x = `Rango Salarial`, y = value  ))+
  geom_col(aes(fill = Género), position = "dodge2")+
  theme_minimal()+
  labs(x = "miles de colones", y = "", fill = " ")+
...
tablaFrecuenciasPensiones <- read_excel("Compania_Pensionados_31_12_2021.xlsx", 
    sheet = "Frecuencias Salarios", range = "H3:L13")

tablaFrecuenciasEdadPensionados <- read_excel("Compania_Pensionados_31_12_2021.xlsx", 
    sheet = "Frecuencias Edades", range = "G3:J14")

tablaPromediosPensionados <- read_excel("Compania_Pensionados_31_12_2021.xlsx", 
    sheet = "Distribución Pensionados", range = "G3:J16")

print(xtable(tablaFrecuenciasPensiones , caption = "Frecuencia absoluta y relativa de montos de pensión rango", align = "|l|c|c|c|c|c|" , label = "etiqueta", digits = 0, include.rownames = FALSE, include.colnames = T, format.args = list(big.mark = c(","), decimal.mark = ".")))


```


### Pirámide poblacional afiliadas

```{r}
#Se genera la pirámide poblacional de los afiliados

pobAfi <- Afiliados_31_12_2021 %>% group_by(Edad, Sexo)%>%
  summarize(Cantidad =  n())
...
plot2 <-pobAfi %>% ggplot(aes(x=Edad, y=Porcentaje*100, fill=Sexo)) +
  geom_bar(stat="identity") +
  ylab('Porcentaje')+
  coord_flip()+
...
#### Pirámide poblacional pensionados

```{r}
#Se genera la piramide poblacional actual de pensionados

pobPen <- Pensionados_31_12_2021 %>% group_by(Edad, SEX)%>%
  summarize(Cantidad =  n())
...
plot3 <- pobPen %>% ggplot(aes(x=Edad, y=Porcentaje*100, fill=SEX)) +
  geom_bar(stat="identity") +
  ylab('Porcentaje')+
  coord_flip()+
  theme_minimal()+scale_fill_brewer(palette='Set1')+
...
# Funciones empleadas en otras funciones
...
`k` (numeric): tiempo transcurrido del momento de valoración y momento esperado de salida.
...
# Funciones para trabajar los decrementos lifecontingencies tiene un bug
#que regresa probabilidades negativas
...
### Función para determinar pension de personas ya pensionadas
...
## Cálculo de pension estocastico
...
### Cálculo para las pensiones de los ya pensionados
...
Se estima el error relativo del modelo estocástico respecto al determinístico, de las pensiones en curso de pago
...
## Función para decrementos de invalidez y vejez
...
## Cálculo de personas activas IVM
...
## Cálculo de costos de liquidación
...
```{r}
PrimasMedias<-rbind.data.frame(
c(1, 723272752165, 0.1986, 0.1854),
c(2, 430005465935, 0.1181, 0.1048),
c(3, 706279705202, 0.1940, 0.1807),
c(4, 419368899461, 0.1152, 0.1019),
c(5, 57861820173,  0.0159, 0.0026),
c(6, 54517956930,  0.0150, 0.0017),
c(7, 104842224865, 0.0288, 0.0155),
c(8, 329186804227, 0.0904, 0.0771),
c(9, 321404395377, 0.0883, 0.0750),
c(10,419368899461, 0.1152, 0.0163))


colnames(PrimasMedias) <-c("Medida","Costo total (CRC)", "Sin reserva", "Con reserva")


PrimasMedias %>%kable( caption = "Costo del plan y prima media nivelada seǵun medida considerada y supuesto sobre la reserva", format.args = list(decimal.mark = ",", big.mark = ".")) %>%
