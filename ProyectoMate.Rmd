...
DecrementosIV2 <- function(Edad, hombre, antiguedad, ultimo_salario, tasa_descuento, tasa_cre_salario, decremento) {
  ...
  PV <- 0
  for (i in edades) {
    # Tamaño del vector y posiciones
    j <- length(0:(115 - i))

    # Vector donde se van guardando los valores sin tener en cuenta el monto de beneficio
    PV <- numeric()

    # Funcion para estimar cada entrada del PV,  que corresponden
    # a cada uno
    funcion_auxiliar <- function(k) {
      ...
      anualidad <- pxt(ListaDeTablasHombres[[(Edad + 1)]], x = i + k, t = vectorMensual) * (v)^(vectorMensual)

      # Suma total del
      suma1 <- sum(anualidad * MontoAPagar)
      PV[k + 1] <- suma1 * v^(k + 1) * pxt(MDListaDeTablasHombres[[Edad + 1]], x = Edad, t = k) * qxt(MDListaDeTablasHombres[[Edad + 1]], x = Edad + k, t = 1, decrement = decremento)
    }
