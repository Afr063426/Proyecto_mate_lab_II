...
pxt1 <- function(tabla, Edad, Diferimiento) {
  px<-numeric()
  if(length(Edad)==1){
    fn<-function(i){
      if (i==0) {
          ...
          return(prod(tabla[Edad+(1:i),6]))
        }
    }
    
    px<-sapply(FUN=fn,X=Diferimiento)
    return(px)
  }else if(length(Edad)>1){
    fn<-function(i){
      if (Diferimiento==0) {
        return(1)
        }else{
          return(prod(tabla[i+(1:Diferimiento),6]))
        }
      }
    px<-sapply(FUN=fn,X=Edad)
    return(px)
  }
}
...
qxt1 <- function(tabla, Edad, Diferimiento, decremento= 5) {
  qx<-numeric()
  if(length(Edad)==1){
    fn<-function(i){
      if (i==0) {
      ...
      return(sum(tabla[Edad+(1:i),decremento]*pxt1(tabla, Edad, (1:i)-1)))
      }}
    qx<-sapply(FUN=fn,X=Diferimiento)
    return(qx)
  }else if(length(Edad)>1){
    fn<-function(i){
      if (Diferimiento==0) {
      ...
      return(sum(tabla[i+(1:Diferimiento),decremento]*pxt1(tabla, i, (1:Diferimiento)-1)))
      }}
    qx<-sapply(FUN=fn,X=Edad)
    return(qx)
  }
}
```
...
costo_estocastico_grupo_etario <- function(edad, salarios, edad_maxima, antiguedades, cant_simulaciones, lista_tablas_decrementos, lista_tablas_mortalidad) {
  ...
  if (cant_pensiones_edad_maxima == 0) {
    ...
    anualidades_edad_maxima <- lapply(X = tiempos_residuales_edad_maxima, FUN = annuity, i = 0.07, m = xi - x, k = 12, type = "due") %>% unlist()
    aguinaldo_edad_maxima <- lapply(X = tiempos_residuales_edad_maxima, FUN = annuity, i = 0.07, m = xi - x, k = 1, type = "immediate") %>% unlist()
  ...
  if (cant_pensiones_edad_maxima < cant_simulaciones) { # esta condición significa que hay otros escenarios
    ...
    cuantiles <- lapply(X = u[u <= FWx[xi - x - 1 + (x + 1)]], FUN = hallar_cuantil, distribucion = FWx[(x+1):116]) %>% unlist()
    ...
    if (identical(posiciones_invalidez, integer(0))) {
      ...
      anualidades_invalidez <- mapply(annuity, i = 0.07, n = tiempos_residuales_invalidez, m = tiempos_hasta_invalidez, k = 12, type = "due", SIMPLIFY = "vector")
      
      aguinaldos_invalidez <- mapply(annuity, i = 0.07, n = tiempos_residuales_invalidez, m = tiempos_hasta_invalidez, k = 1, type = "immediate", SIMPLIFY = "vector")
    ...
    if (identical(posiciones_jubilacion, integer(0))) {
      ...
      anualidades_jubilacion <- mapply(annuity, i = 0.07, n = tiempos_residuales_jubilacion, m = tiempos_hasta_jubilacion, k = 12, type = "due", SIMPLIFY = "vector")
      aguinaldos_jubilacion <- mapply(annuity, i = 0.07, n = tiempos_residuales_jubilacion, m = tiempos_hasta_jubilacion, k = 1, type = "immediate", SIMPLIFY = "vector")
    ...
    if (identical(posiciones_muerte, integer(0))) {
      ...
      anualidades_muerte <- mapply(annuity, i = 0.07, n = 10, m = tiempos_hasta_muerte, k = 12, type = "due", SIMPLIFY = "vector")
      aguinaldos_muerte <- mapply(annuity, i = 0.07, n = 10, m = tiempos_hasta_muerte, k = 1, type = "immediate", SIMPLIFY = "vector")
...
proc.time()-t
archivo <- file("Resultados estocástico grupal Hombres 10000.txt")
...
proc.time()-t
archivo <- file("Resultados estocástico grupal Mujeres 10000.txt")
...
costo_afiliado_estocastico <- function(edad, salario, edad_maxima, antiguedad, cant_simulaciones, lista_tablas_decrementos, lista_tablas_mortalidad) {
  ...
  if(identical(cant_pensiones_edad_maxima, numeric(0))){
  ...
  costo_estocastico <- sum(costos_edad_maxima, costos_invalidez, costos_jubilacion, costos_muerte)/ cant_simulaciones 
  
  return(costo_estocastico)
}
```

```{r Prueba estocástico individual}
set.seed(10)
N <- 10000
...
# calculo de pension para mujeres

MontoParaMujeresDeterministicoM <-
  future_mapply(
    DecrementoM,
    afil_mujeres$Edad,
    FALSE,
    afil_mujeres$Antiguedad,
    afil_mujeres$Salario,
    0.07,
    56
  )
sum(unlist(MontoParaMujeresDeterministicoM))



MontoParaMujeresDeterministicoI <-
  mapply(
    DecrementosIV,
    afil_mujeres$Edad[1],
    FALSE,
    afil_mujeres$Antiguedad[1],
    afil_mujeres$Salario[1],
    0.07,
    56,
    1
  )
sum(MontoParaMujeresDeterministicoI)
...
sum(MontoParaMujeresDeterministicoV)

sum(MontoParaMujeresDeterministicoV, MontoParaMujeresDeterministicoI, MontoParaMujeresDeterministicoM)
```
...
DecrementoM <-
  function(Edad, hombre, antiguedad, ultimo_salario, tasa_descuento, tasa_cre_salario) {
    ...
    if (hombre) {
        ...
        ultimo_salario * ((1 + 0.04)*(1+0.025))^(0:(85-Edad))*12*pxt1(MDListaDeTablasHombres[[(85-Edad)]], x = (85-Edad), t = 1)
        

    } else {
       ultimo_salario * ((1 + 0.04)*(1+0.025))^(0:(85-Edad))*12*pxt1(MDListaDeTablasMujeres[[(77 - Edad)]], x = (77 - Edad), t = 1)
    }
